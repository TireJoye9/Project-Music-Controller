...
class RoomView(generics.ListAPIView): #allows us to view all different rooms and create the rooms too
...
class GetRoom(APIView):
    ...
    def get(self, request, format=None):
        ...
        if code != None:
            ...
            if len(room) > 0:
                #taking data and extracting from room serializer
...
#allows us to override default methods e.g put
...
class UserInRoom(APIView):
    def get(self, request, format=None):
        if not self.request.session.exists(self.request.session.session_key):                self.request.session.create()
        data = {
